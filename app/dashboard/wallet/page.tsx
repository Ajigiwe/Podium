                ...
                {loading ? <div className="p-8 text-center text-sm">Loading...</div> : txs.length===0 ? <div className="p-8 text-center text-[11px] text-[#8888A8] uppercase tracking-widest">No transactions yet</div> : (
                    <div className="divide-y divide-[#DDE0F0]">
                        {txs.map(t=>{
                            ...
                            const ref = (t.paystackReference || '') as string;
                            const sess = (t.sessionId || '') as string;
                            const ch = (t.paymentChannel || '') as string;
                            return (
                                <div key={t.id} className="px-6 py-4 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-[13px] font-bold text-[#0D0D1A]">{t.type==='top_up' ? 'Top-up' : t.type==='refund' ? 'Refund' : t.type==='session_payment' ? `Class payment` : 'Transaction'} <span className="text-[11px] text-[#8888A8]">{sess && sess !== 'wallet_topup' ? `· ${sess.slice(0,8)}` : ''}</span></div>
                                            <div className="text-[11px] text-[#8888A8]">{t.createdAt ? new Date(t.createdAt).toLocaleString() : ''} · {ch} · {ref ? ref.slice(0,12) : ''}</div>
                                        </div>
                                    </div>
