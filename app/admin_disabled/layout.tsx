...
import { GraduationCap, LayoutDashboard, Settings, LogOut, Menu, X, Sparkles, ArrowLeft, Shield } from 'lucide-react';
            ...
            <aside className="hidden lg:flex w-64 flex-col fixed inset-y-0 bg-white border-r border-[#DDE0F0] z-30">
                ...
                <nav className="flex-1 px-6 space-y-8 mt-10">
                    <div className="space-y-1">
                        ...
                        <Link 
                            href="/admin" 
                            className={`flex items-center gap-3 px-4 py-3 rounded-md text-[11px] font-bold uppercase tracking-widest transition-all ${pathname === '/admin' ? 'bg-[#1845D4] text-white shadow-lg shadow-blue-600/10' : 'text-[#8888A8] hover:text-[#0D0D1A] hover:bg-[#F5F6FA]'}`}
                        >
                            <Shield className="w-4 h-4" /> System Oversight
                        </Link>
                    </div>
                ...
                <div className="fixed inset-0 z-50 lg:hidden">
                    ...
                    <div className="fixed inset-y-0 left-0 w-72 bg-white shadow-2xl flex flex-col p-6 animate-in slide-in-from-left duration-300">
                        ...
                        <nav className="flex-1 space-y-4">
                            <Link href="/admin" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-4 px-5 py-4 rounded-md text-[11px] font-bold uppercase tracking-widest bg-[#1845D4] text-white shadow-lg shadow-blue-600/10"><Shield className="w-5 h-5" /> Oversight</Link>
                            <Link href="/dashboard" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-4 px-5 py-4 rounded-md text-[11px] font-bold uppercase tracking-widest text-[#8888A8]"><ArrowLeft className="w-5 h-5" /> Return</Link>
                        </nav>
