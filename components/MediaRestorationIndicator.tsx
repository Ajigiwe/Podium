...
export const MediaRestorationIndicator = () => {
    const { restorationStatus, retryRestoration, isWaitingForInteraction } = useMediaPersistence();
                ...
                <div className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 animate-in fade-in slide-in-from-top-4 duration-300">
                    ...
                    <span className="text-sm font-medium">
                        {isWaitingForInteraction
                            ? "Waiting for you to click to restore media..."
    ...
    if (restorationStatus === 'error') {
        return (
            <div className="fixed top-24 left-1/2 transform -translate-x-1/2 z-[100]">
                <div className="bg-yellow-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-3 animate-in fade-in slide-in-from-top-4 duration-300">
                    <div className="flex items-center gap-2">
                        <AlertCircle className="w-4 h-4" />
                        <span className="text-sm font-medium">Could not restore camera/mic.</span>
                    </div>
                    <button
                        onClick={() => retryRestoration()}
                        className="bg-white/20 hover:bg-white/30 px-2 py-1 rounded text-xs font-bold transition-colors border border-white/40"
                    >
                        Retry
                    </button>
                </div>
